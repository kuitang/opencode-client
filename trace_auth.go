...
func main() {
	...
	if err := json.Unmarshal(authData, &authConfig); err != nil {
		log.Fatalf("Failed to parse auth JSON: %v", err)
	}
	...
	if err != nil {
		log.Fatalf("Failed to marshal auth config: %v", err)
	}

	if _, err := tmpFile.Write(authDataFormatted); err != nil {
		log.Fatalf("Failed to write temp auth file: %v", err)
	}
	tmpFile.Close()
	...
	if err := json.NewDecoder(resp.Body).Decode(&providersResp); err != nil {
		log.Fatalf("Failed to decode providers response: %v", err)
	}
